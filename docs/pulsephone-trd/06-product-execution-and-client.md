...
外部curl安装脚本在`self install --json`成功后从结构化result提取`launcherPath`，仅当其父目录不在当前`PATH`时打印可复制配置段。检测shell以用户login shell为线索，不能把安装脚本的`/bin/bash`解释为当前交互shell；zsh使用`${ZDOTDIR:-$HOME}/.zshrc`，bash为login和non-login分别维护`.bash_profile`和`.bashrc`。配置段只在用户粘贴执行时写入当前用户的普通配置文件，重复执行不重复追加或重复插入PATH；拒绝symlink、非regular和非本用户已有目标。安装脚本本身不写profile，不请求sudo，不修改父shell环境；不支持的shell或home不一致时只提示绝对入口，不猜配置。此提示属于外部安装器，不改变`self.install`结果schema或事务语义。
