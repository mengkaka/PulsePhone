...
func run() int {
	return helperapp.RunOwned(os.Stdin, func(ctx context.Context) int {
		return runWithContext(os.Args[1:], os.Stdin, os.Stdout, os.Stderr, ctx, processidentity.CurrentProcessStartIdentity, func(runtimeEpoch, executorGeneration uint64, rawTransportUDID string, connectionEpoch uint64, serviceNames map[string]string) (coreDeviceBackend, error) {
...
func runWith(arguments []string, stdin io.Reader, stdout io.Writer, stderr io.Writer, currentProcessStartIdentity func() (string, error), newBackend coreDeviceBackendFactory) int {
	return runWithContext(arguments, stdin, stdout, stderr, context.Background(), currentProcessStartIdentity, newBackend)
}

func runWithContext(arguments []string, stdin io.Reader, stdout io.Writer, stderr io.Writer, ctx context.Context, currentProcessStartIdentity func() (string, error), newBackend coreDeviceBackendFactory) int {
